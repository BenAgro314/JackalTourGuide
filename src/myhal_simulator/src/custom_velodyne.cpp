...
namespace gazebo
{
    ...
    {   
        ...
        for (auto act: this->actors){
            ...
            auto min = ignition::math::Vector3d(act->WorldPose().Pos().X() - 0.4, act->WorldPose().Pos().Y() - 0.4, 0);
            auto max = ignition::math::Vector3d(act->WorldPose().Pos().X() + 0.4, act->WorldPose().Pos().Y() + 0.4, 0);
