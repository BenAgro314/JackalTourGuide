...
FramesAndTraj DataProcessor::GetData(){
    ...
    for (auto msg: view){
        if (msg.getTopic() == this->gt_topic){
            ...
            if (pose != nullptr)
                trajectory.push_back(TrajPoint(ignition::math::Pose3d(pose->pose.pose.position.x, pose->pose.pose.position.y, pose->pose.pose.position.z, pose->pose.pose.orientation.x, pose->pose.pose.orientation.y, pose->pose.pose.orientation.z, pose->pose.pose.orientation.w), pose->header.stamp.toSec()));
            continue;
        } 
