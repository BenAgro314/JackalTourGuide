...
The first goal of the project is to produce pointcloud and pose data of the Jackal as it completes predetermined tours around Myhal. This data is then used to train a novel deep-learning SLAM algorithm to distinguish between objects that are moving (eg. humans), objects that can be moved (eg. tables), and immovable objects (eg. walls). The creation of a diverse enviroments and situations within the 5th floor of Myhal (within a range of desired provided parameters) and the process of Jackal touring are automatted. This allows for increased amounts of data to be collected without manual intervention.
